...
import { localizeTruthOrDarePrompt } from '../data/promptCatalog';
  ...
  if (!choice || !prompt) {
    return (
      <div className="rounded-[1.8rem] border border-dashed border-white/10 bg-white/[0.03] p-5 text-sm leading-7 text-slate-400 sm:p-6">
        {t('game.challengePlaceholder')}
      </div>
    );
  }

  const fallbackAvatar = selectedPlayerName?.slice(0, 1).toUpperCase() || 'P';
  const localizedChoice = choice === 'truth' ? t('common.truth') : t('common.dare');
  const localizedPrompt = localizeTruthOrDarePrompt(prompt, language);
      ...
      {roundResult ? (
        <p className="mt-4 inline-flex rounded-full border border-white/10 bg-white/[0.06] px-3 py-2 text-sm text-slate-200">
          {roundResult.action === 'completed'
            ? t('game.roundScoredComplete', { points: roundResult.points })
            : t('game.roundScoredSkip', { points: roundResult.points })}
        </p>
      ) : null}
    </div>
  );

  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-md">
      <div className="glass-panel w-full max-w-lg p-5 sm:p-8">
        ...
        <p className="section-kicker">{t('common.selectedPlayer')}</p>
        ...
        <p className="mt-3 text-sm leading-7 text-slate-300">
          {isCurrentPlayerTurn
            ? t('choiceModal.currentPlayerText')
            : t('choiceModal.otherPlayerText', { name: selectedPlayer.name })}
        </p>
        <div className="mt-4 rounded-[1.55rem] border border-white/10 bg-white/[0.05] px-4 py-3 text-center">
          <p className="text-xs uppercase tracking-[0.25em] text-slate-400">
            {t('common.autoPicksIn')}
          </p>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          ...
          >
            <span className="text-base">{t('common.truth')}</span>
            <span className="text-xs font-medium text-white/75">{t('choiceModal.truthHint')}</span>
          </Button>
          <Button
          ...
          >
            <span className="text-base">{t('common.dare')}</span>
            <span className="text-xs font-medium text-slate-200">{t('choiceModal.dareHint')}</span>
          </Button>
        </div>
      </div>
    </div>

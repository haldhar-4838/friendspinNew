import { useLanguage } from '../context/LanguageContext';

function Leaderboard({ players = [], compact = false }) {
  const { t } = useLanguage();
  const sortedPlayers = [...players].sort((firstPlayer, secondPlayer) => {
    if ((secondPlayer.score || 0) !== (firstPlayer.score || 0)) {
      return (secondPlayer.score || 0) - (firstPlayer.score || 0);
    }

    return firstPlayer.name.localeCompare(secondPlayer.name);
  });

  return (
    <div className="space-y-3">
      {sortedPlayers.map((player, index) => (
        <div
          key={player.id}
          className={[
            'glass-outline flex items-start justify-between gap-3 rounded-[1.7rem]',
            index === 0 ? 'border-bubblegum/20 bg-bubblegum/[0.08]' : '',
            compact ? 'px-3.5 py-3' : 'px-4 py-4',
          ].join(' ')}
        >
          <div className="flex min-w-0 items-start gap-3">
            <div
              className={[
                'flex shrink-0 items-center justify-center rounded-[1.1rem] bg-gradient-to-br from-bubblegum/70 via-neon/70 to-aurora/70 text-white',
                compact ? 'h-10 w-10 text-sm font-semibold' : 'h-11 w-11 text-xl',
              ].join(' ')}
            >
              {player.avatar || player.name.slice(0, 1).toUpperCase()}
            </div>
            <div className="min-w-0">
              <p
                className={[
                  'break-words font-medium leading-5 text-white',
                  compact ? 'text-sm' : '',
                ].join(' ')}
              >
                {index === 0 ? t('leaderboard.winner') : `#${index + 1}`} {player.name}
              </p>
              <p className="mt-1 text-[11px] uppercase tracking-[0.25em] text-slate-400">
                {player.isHost ? t('common.host') : t('common.player')}
              </p>
            </div>
          </div>
          <div className="shrink-0 text-right">
            <p
              className={[
                'font-display font-bold text-white',
                compact ? 'text-xl' : 'text-2xl',
              ].join(' ')}
            >
              {player.score || 0}
            </p>
            <p className="text-[11px] uppercase tracking-[0.25em] text-slate-400">
              {t('common.points')}
            </p>
          </div>
        </div>
      ))}
    </div>
  );
}

export default Leaderboard;
